...
The files were loaded into R using the the "Files" tab and finding the files through my directory as indicated below. I titled each data by the bone it reffered to and viewed the head of the data.
...
Here I have loaded the tree data naming them by bone name or complete and adding "tree" in the name.

Additional trees I have replicated that are not found in any fugures on the paper can be found below in the "Extra" section.
...
#Here I visualize the tree as a Cladogram (or rectangular layout) as the paper does. Here I rotate each of the nodes individually and modify the lengths of the branches from the tree.
...
#after rotating nodes, L1 is in the same order as the paper
plot(L1)

#now we modify branch legth to 1
...
#Here I visualize tree and call it LucasTree
LucasTree <- ggtree(phylocomplete, branch.length='none') + geom_tiplab() 
...
LucasTree <- ggtree(complete, branch.length='none', layout="rectangular") + geom_tiplab() + theme_tree2()

plot(complete)
ggtree(complete) + geom_tiplab() 
Arbol <- ggtree(complete, branch.length='none', layout="rectangular") + geom_tiplab()
plot(Arbol)
plot(LucasTree)
plot(complete)

plot(complete, use.edge.length=FALSE)
nodelabels()


plot(Rtree, use.edge.length=FALSE)
nodelabels()


plot(Rtree)
(nodelabels(LucasTree)) 


RRtree<-allRotations(tree)
plot(RRtree)

1+1
rotateNodes(LucasTree, 29)
###

(ReorderData(LucasTree))

orderedTree<-ReorderData(LucasTree, complete, taxa.names = "species")

plot(orderedTree)

(rotate(LucasTree, node = 9))

plot(LucasTree, use.edge.length=FALSE)

nodelabels()
#complete[,c(2,4:6)]<-log(complete[,c(2,4:6)]); complete[,3]<-log1p(complete[,3])
```

In order to make my life easier I am changing the names of the Resting Metabolic Rate columns to "RMR"
...
colnames(femur)
```
Here I combine all RMR all bone datasets. I combine all Species from all bone datasets. I cbind Species and RMR, removing NA's and duplicates.
...
#USE PHYTOOLS
#TRY 2
#Here I plot all of the RMR and Species,  using shapes and colors like the paper.
...
The data above only contains the data provided in the data from dryad.
In order to have all of the data reported on Figure 1, I have manually added all of the Table 1 data below, and cbind them into a dataframe called missingspecies. I then join this data with the previous data above. into a dataframe called full data.
```{r}
