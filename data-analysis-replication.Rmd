...
Here I have loaded the tree data naming them by the component (tibia, humerus, femus, or complete) and adding "tree" in the name.
...
# As a reminder this is how to load a NEXUS file
completetree <- read.nexus(file = "/Users/jessicavaldes/Desktop/R2024/FINAL.PROJECT/Treecomplete.trees.nex")

# Here I convert the femurtree to a phylo object in order to view using ggtree
phylocomplete <- as.phylo(completetree)

# Here I plot the phylo tree using ggtree, add tip labels (shows the species name in this case) and apply a tree-like theme, and name it Main Tree
...
Here I rotate each of the nodes individually. (Note: I use the phylo  object in order to be able to rotate the nodes using the rotate.multi function).

Lastly, I visualize the tree as a Cladogram (or rectangular layout) as the paper does by modifying the lengths of the branches from the tree.
...
#I have named the rotated tree L1 which is now in the same order as the paper
plot(L1)

#now we modify branch legth to 1 (for my convenience I have named the rectangle cladogram LL1)
LL1<-compute.brlen(L1, length =1)

#LL1 now matched Figure 1 of the paper.
plot(LL1)
...
In order to make my life easier when combining this data I am changing the names of the Resting Metabolic Rate columns to "RMR". I then check the column names to make sure I have the correct new column name titles.
...
#now that I have all of the data togehter I can greate the plot.
#Here I plot RMR on the x-axis and Species on the Y-axis
#I make sure the shapes are squares
...
Here I realize that the data above only contains the data provided in the data from dryad.

In order to have all of the data reported on Figure 1, I have manually added all of the Table 1 data from the paper below, and cbind them into a dataframe called missingspecies. I then join this data with the previous data above. into a dataframe called full data.
```{r}
#here i add the data that was provided in the paper table but not in the data on dryad but that is shown on Figure 1.
...
#change the shape to 22 as the squares are outlined in the paper
#save image as FIGURE1
...
#Now we add the whiskers
#making sure to take out any NA's and I created a new dataset called "whiskers" for that



#Species order

#Sorder <- c("Pleurodeles waltl", "Microcebus murinus", "Cavia porcellus", "Mus musculus", "Zootoca vivipara", "Podarcis muralis", "Varanus exanthematicus", "Varanus niloticus", "Chelodina oblonga", "Pelodiscus sinensis","Trachemys scripta", "Prolacerta broomi", "Proterosuchus fergusi", "Garjainia madiba", "Euparkeria capensis", "Rutiodon carolinensis", "Crocodylus niloticus", "Postosuchus kirkpatricki", "Calyptosuchus wellesi", "Lesothosaurus diagnosticus", "Maiasaura peeblesorum", "Thecodontosaurus antiquus", "Coelophysis bauri", "Allosaurus fragilis", "Lourinhanosaurus antunesi", "Troodon formosus", "Anas platyrhynchos", "Gallus gallus")
...
In order to make my life easier I am changing the names of the Resting Metabolic Rate columns to "RMR". I then check the column names to make sure I have the correct new column name titles.
